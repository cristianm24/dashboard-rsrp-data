...
# Columnas que identifican una hoja de datos de plan de trabajo
_COLS_PLAN_IDENTIFIERS = ["AGENTE", "META ALTA NAT (>$2000)", "EJEC ALTA NAT", "EJE ALTA TOTAL"]
...
def _scan_all_sheets(xl):
    ...
    Retorna un dict con:
      - 'plan': lista de (sheet_name, score, cols) — hojas de datos de plan
      - 'cierre': lista de sheet_name — hojas de cierre mensual
      - 'other': lista de sheet_name — resto
    """
    result = {"plan": [], "cierre": [], "other": []}
    for sheet in xl.sheet_names:
        try:
            # Try header at row 0 first
            preview = pd.read_excel(xl, sheet_name=sheet, header=0, nrows=2)
            cols_r0 = [str(c).strip() for c in preview.columns]
            score_r0 = sum(1 for c in COLUMNAS_REQUERIDAS if c in cols_r0)

            # Try header at row 4 (like LIKE SUR sheet)
            preview4 = pd.read_excel(xl, sheet_name=sheet, header=4, nrows=2)
            cols_r4 = [str(c).strip() for c in preview4.columns]
            score_r4 = sum(1 for c in COLUMNAS_REQUERIDAS if c in cols_r4)

            best_score = max(score_r0, score_r4)
            best_cols  = cols_r0 if score_r0 >= score_r4 else cols_r4
            best_header= 0 if score_r0 >= score_r4 else 4

            if best_score >= 3:
                result["plan"].append({
                    "sheet": sheet, "score": best_score,
                    "cols": best_cols, "header_row": best_header
                })
            elif "ID POS" in cols_r0 or ("ID" in cols_r0 and len(cols_r0) <= 5):
                result["cierre"].append(sheet)
            else:
                result["other"].append(sheet)
        except Exception:
            result["other"].append(sheet)

    # Sort plan sheets by score descending
    result["plan"].sort(key=lambda x: x["score"], reverse=True)
    return result
...
def _find_all_plan_sheets(xl):
    """
    Retorna lista de todas las hojas válidas de plan — para selector multi-mes.
    Cada item: {"sheet": name, "score": n, "cols": [...], "header_row": n}
    """
    return _scan_all_sheets(xl)["plan"]


def _find_cierre_sheets(xl):
    """Retorna lista de hojas de cierre detectadas."""
    return _scan_all_sheets(xl)["cierre"]
...
def _load_from_xl(xl, source_label):
    """
    Core loader: given an open ExcelFile, detect sheets by content and load.
    Returns (df_det, df_cierre, df_plan, info_dict).
    Never raises — always returns a result.
    """
    available = xl.sheet_names

    # 1. Detect main plan sheet
    sheet_name, header_row, err = _find_detail_sheet(xl)
    if err:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
            "found": False, "message": err
        }

    # 2. Load and process main sheet
    try:
        df_det = pd.read_excel(xl, sheet_name=sheet_name, header=header_row)
        df_det, faltantes, nuevas = _process_claro_df(df_det)
        if faltantes:
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
                "found": False,
                "message": (f"Hoja '{sheet_name}' encontrada pero faltan columnas: "
                            f"{', '.join(faltantes)}")
            }
    except Exception as e:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
            "found": False, "message": f"Error leyendo '{sheet_name}': {e}"
        }

    # 3. Detect cierre sheet dynamically
    df_cierre = pd.DataFrame()
    cierre_sheets = _find_cierre_sheets(xl)
    for cs in cierre_sheets:
        try:
            df_c = pd.read_excel(xl, sheet_name=cs, header=0)
            df_c.columns = [str(c).strip() for c in df_c.columns]
            if len(df_c.columns) >= 3:
                df_c = df_c.iloc[:, :3].copy()
                df_c.columns = ["ID_POS", "MAR_ALTAS", "MAR_INGRESOS"]
                df_c["MAR_ALTAS"]    = pd.to_numeric(df_c["MAR_ALTAS"],    errors="coerce")
                df_c["MAR_INGRESOS"] = pd.to_numeric(df_c["MAR_INGRESOS"], errors="coerce")
                df_cierre = df_c
            break
        except Exception:
            continue

    # 4. Detect any secondary plan sheet (resumen por agente)
    df_plan = pd.DataFrame()
    all_plan = _find_all_plan_sheets(xl)
    for ps in all_plan:
        if ps["sheet"] != sheet_name:
            try:
                df_plan = pd.read_excel(xl, sheet_name=ps["sheet"],
                                        header=ps["header_row"])
    ...
    return df_det, df_cierre, df_plan, {
        ...
        "all_plan_sheets": [p["sheet"] for p in all_plan],
        "cierre_sheets": cierre_sheets,
        "dia_corte": _dia_corte,
    }


def load_claro_data_from_path(path):
    """Carga desde ruta en disco."""
    try:
        xl = pd.ExcelFile(path, engine="xlrd" if str(path).lower().endswith(".xls") else "openpyxl")
    except Exception as e:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
...
def load_claro_data_from_upload(uploaded_file):
    ...
    try:
        ...
        raw = st.session_state.get("claro_file_bytes", b"")
        fname = st.session_state.get("claro_file_name", fname)

    if not raw:
        raw = st.session_state.get("claro_file_bytes", b"")
    if not raw:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
            "found": False, "message": "No se pudieron leer los bytes del archivo."
        }
    ...
    if fname.endswith(".csv"):
        try:
            try:
                df = pd.read_csv(_io.BytesIO(raw), sep=None, engine="python", encoding="utf-8", on_bad_lines="skip")
            except Exception:
                df = pd.read_csv(_io.BytesIO(raw), sep=",", encoding="latin-1", on_bad_lines="skip")
...
def load_claro_data():
    ...
    return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {
        "found": False,
        "message": "Sin archivo. Sube el Excel del mes en el sidebar."
    }


def render_instructivo():
    """Instructivo — diseño consistente con tema claro premium."""
    ...
    <div style="animation:fadeDown .3s ease both;display:flex;align-items:center;gap:16px;
        ...
        <div>
            <div style="font-size:1.3rem;font-weight:950;color:var(--text-primary);
                letter-spacing:-.3px;">Gu&#237;a de uso del dashboard</div>
            <div style="font-size:.80rem;color:var(--text-muted);margin-top:2px;">
                Claro Colombia &middot; Inteligencia Comercial y de Red</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Shared styles ──────────────────────────────────────────────────────────
    _card = "background:white;border:1px solid var(--border);border-radius:var(--radius-md);padding:22px 24px;box-shadow:var(--shadow-sm);margin-bottom:12px;animation:fadeUp .35s ease both;"
    _tag_req  = "<span style='background:var(--accent);color:white;font-size:.58rem;font-weight:800;padding:2px 7px;border-radius:99px;margin-left:6px;vertical-align:middle;'>REQUERIDO</span>"
    _tag_opt  = "<span style='background:var(--bg-sidebar);color:var(--text-muted);font-size:.58rem;font-weight:700;padding:2px 7px;border-radius:99px;border:1px solid var(--border);margin-left:6px;vertical-align:middle;'>OPCIONAL</span>"
    _step_style = "width:20px;height:20px;background:var(--text-primary);color:white;font-size:.62rem;font-weight:900;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;"
    ...
    def _file_row(icon, name, tag_html, desc):
        return (
            ...
            f"<div><div style='font-size:.76rem;font-weight:800;color:var(--text-primary);'>"
            f"{name}{tag_html}</div>"
    ...
    def _tab_row(icon, name, desc):
        return (
            ...
            f"<div><div style='font-size:.76rem;font-weight:800;color:var(--text-primary);'>{name}</div>"
            f"<div style='font-size:.70rem;color:var(--text-muted);'>{desc}</div></div></div>"
        )
    ...
    with col_i1:
        st.markdown(
            ...
            f"<div style='width:30px;height:30px;background:rgba(37,99,235,0.10);border-radius:8px;"
            f"display:flex;align-items:center;justify-content:center;'>"
            f"<svg width='15' height='15' viewBox='0 0 24 24' fill='none' stroke='#2563EB' stroke-width='2.2'"
            ...
            f"<div style='font-size:.92rem;font-weight:900;color:#2563EB;'>Vista Red y Mercado</div></div>"
            f"<div style='font-size:.78rem;color:var(--text-muted);line-height:1.7;margin-bottom:14px;'>"
            f"Analiza se&#241;al RSRP de todos los operadores por c&#243;digo postal. "
            f"Comparativo de cuota de mercado y captaci&#243;n de altas.</div>"
        ...
        # Tabs card
        st.markdown(
            f"<div style='{_card}background:var(--bg);'>"
            f"<div style='font-size:.68rem;font-weight:900;color:#2563EB;text-transform:uppercase;"
            f"letter-spacing:.4px;margin-bottom:10px;'>5 tabs disponibles</div>"
            + _tab_row("🔵","Resumen","Estado global de se&#241;al y posici&#243;n comercial")
            + _tab_row("📡","Operadores","Ranking de se&#241;al entre operadores")
            + _tab_row("🗺","Territorio","CP cr&#237;ticos y zonas prioritarias")
            + _tab_row("📈","Variaci&#243;n","Cambio de se&#241;al en el tiempo")
            + _tab_row("🏪","Mercado","Cuota, captaci&#243;n y an&#225;lisis competitivo CP a CP")
            + "</div>",
            unsafe_allow_html=True
        )
    ...
    with col_i2:
        st.markdown(
            ...
            f"<div style='display:flex;align-items:center;gap:8px;margin-bottom:12px;'>"
            f"<div style='width:30px;height:30px;background:var(--accent-soft);border-radius:8px;"
            f"display:flex;align-items:center;justify-content:center;'>"
            f"<svg width='15' height='15' viewBox='0 0 24 24' fill='none' stroke='var(--accent)' stroke-width='2.2'"
            ...
            f"<div style='font-size:.92rem;font-weight:900;color:var(--accent);'>Vista Agentes Claro</div></div>"
            f"<div style='font-size:.78rem;color:var(--text-muted);line-height:1.7;margin-bottom:14px;'>"
            f"Seguimiento del plan de trabajo mensual: metas, ejecuci&#243;n semanal, PDVs, "
            f"asesores y cuota de altas por agente y circuito.</div>"
            f"<div style='font-size:.66rem;font-weight:900;color:var(--text-muted);text-transform:uppercase;"
            f"letter-spacing:.4px;margin-bottom:8px;'>Archivo</div>"
            + _file_row("📋","Plan de trabajo mensual .xlsx",_tag_req,
                ...
                "&mdash; el sistema detecta la hoja por su contenido.<br>"
                "Columnas: AGENTE &middot; META ALTA NAT &middot; EJEC ALTA NAT &middot; EJE ALTA TOTAL")
            + f"<div style='background:var(--bg);border:1px solid var(--border);border-radius:10px;"
            f"padding:10px 12px;margin-bottom:12px;font-size:.72rem;color:var(--text-muted);line-height:1.55;'>"
            f"Si el archivo tiene <b style='color:var(--text-primary);'>m&#250;ltiples hojas</b> (una por mes), "
            f"el sistema las detecta todas y muestra un selector de periodo en el sidebar.</div>"
            f"<div style='font-size:.66rem;font-weight:900;color:var(--text-muted);text-transform:uppercase;"
            f"letter-spacing:.4px;margin:12px 0 7px;'>C&#243;mo cargar</div>"
            ...
            + _step_row(3,"Si hay varios meses, aparece un selector de periodo autom&#225;ticamente")
            + "</div>",
            unsafe_allow_html=True
        )
        # Tabs card
        st.markdown(
            f"<div style='{_card}background:var(--bg);'>"
            f"<div style='font-size:.68rem;font-weight:900;color:var(--accent);text-transform:uppercase;"
            f"letter-spacing:.4px;margin-bottom:10px;'>5 tabs disponibles</div>"
            + _tab_row("↗","&#191;C&#243;mo vamos?","Resultado del mes &middot; metas vs ejecuci&#243;n")
            + _tab_row("◈","&#191;Qui&#233;n cumple?","Ranking y cumplimiento por agente")
            + _tab_row("◎","La brecha","PDVs cr&#237;ticos y capacidad de mejora")
            + _tab_row("∿","El ritmo","Curva semanal S1 &#8594; S4")
            + _tab_row("◉","Oportunidades","Cuota de altas y se&#241;al RSRP")
            + "</div>",
            unsafe_allow_html=True
        )
...
def render_claro_view():
    ...
    # -------------------------------------------------------
    with tc1:
        # Corte dinámico — calculado desde los datos (S1-S4 con información)
        _DIA_CORTE = info.get("dia_corte", 30)
        _DIAS_MES  = 30
...
try:
    if _rsrp_available:
        ...
        operator_cols = []
        territorial_df = pd.DataFrame()
        territorial_info = {}
        business_long = pd.DataFrame()
        market_info = {}
        altas_info = {}
...
if _uploaded_claro is not None:
    import io as _upio
    # Read bytes ONCE immediately
    _uploaded_claro.seek(0)
    _claro_bytes = _uploaded_claro.read()
    _claro_name  = _uploaded_claro.name
    st.session_state["claro_file_bytes"] = _claro_bytes
    st.session_state["claro_file_name"]  = _claro_name
    st.session_state["claro_uploaded_file"] = _uploaded_claro
    try:
        # Use BytesIO from stored bytes — never touch the original stream again
        _buf = _upio.BytesIO(_claro_bytes)
        _is_xls = _claro_bytes[:2] == bytes([0xD0, 0xCF])
        _xleng  = "xlrd" if _is_xls else "openpyxl"
        _xl_prev = pd.ExcelFile(_buf, engine=_xleng)
        _sheet_prev, _header_prev, _err_prev = _find_detail_sheet(_xl_prev)
        if _err_prev:
            st.sidebar.error(f"⚠️ {_err_prev}")
        else:
            _buf2 = _upio.BytesIO(_claro_bytes)
            _preview = pd.read_excel(_buf2, sheet_name=_sheet_prev, header=_header_prev, nrows=3, engine=_xleng)
            ...
            _buf3 = _upio.BytesIO(_claro_bytes)
            _xl_prev2 = pd.ExcelFile(_buf3, engine=_xleng)
            _all_plan_c = _find_all_plan_sheets(_xl_prev2)
            if _faltantes_c:
                ...
                st.sidebar.success(f"✅ {_claro_name}{_meses_txt}")
    except Exception as _e:
        st.sidebar.error(f"Error leyendo archivo: {_e}")
else:
